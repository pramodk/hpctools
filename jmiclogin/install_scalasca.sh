...
export INSTALL_DIR=$HOME/workarena/systems/jknc/softwares/install/profilers/scalasca_x86
export SOURCE_DIR=$HOME/workarena/systems/jknc/softwares/sources/profilers/scalasca_x86
...
do
    ...
    case "$package" in
        ...
        sioni)
            ...
            ./configure --prefix=$SION_INSTALL_DIR --compiler=${compiler} --mpi=intel2  --enable-pthreads
            cd build-mic
        ...
        qti)
            wget --no-check-certificate https://download.qt.io/archive/qt/4.8/4.8.5/qt-everywhere-opensource-src-4.8.5.tar.gz
        ...
        scorepi)
            ...
            ./configure --prefix=$SCOREP_INSTALL_DIR --with-nocross-compiler-suite=${compiler} --with-mpi=intel --with-otf2=$OTF2_INSTALL_DIR/bin --with-opari2=$OPARI2_INSTALL_DIR/bin --with-cube=$CUBE_INSTALL_DIR --with-papi-header=$PAPI_INSTALL_DIR/include --with-papi-lib=$PAPI_INSTALL_DIR/lib
            make -j12
            make check
            make install
            ;;
        ...
        scalasca)
            ...
            ./configure --prefix=$SCALASCA_INSTALL_DIR --with-mpi=intel --with-otf2=$OTF2_INSTALL_DIR --with-cube=$CUBE_INSTALL_DIR --with-sionconfig=$SION_INSTALL_DIR/bin/sionconfig --with-libz=$ZLIB_INSTALL_DIR MPI_CXXFLAGS="-DMPICH_IGNORE_CXX_SEEK -DMPICH_SKIP_MPICXX" --disable-silent-rules  CC=icc CXX=icpc
